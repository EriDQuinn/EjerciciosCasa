...
void myData() //Recordemos que antes aquí teníamos un cubo, pero ahora hay un plano.
{
	...
	unsigned char *data = stbi_load("Texturas/marmol.jpg", &width, &height, &nrChannels, 0); 
	if (data)
	{
	...
	data = stbi_load("Texturas/mar_dor.png", &width, &height, &nrChannels, 0);
	if (data)
	{
...
void display(void)
{
	...
	model = glm::scale(model, glm::vec3(2.5,.5 ,2.5));
	projectionShader.setMat4("model", model);
	projectionShader.setVec3("aColor", glm::vec3(1.0f, 1.0f, 1.0f));
	glDrawElements(GL_TRIANGLES, 36, GL_UNSIGNED_INT, 0);
	model = glm::mat4(1.0f);

	model = glm::translate(model, glm::vec3(0.0f, 2.750f, 0.0f));
	model = glm::scale(model, glm::vec3(1,5,1));
	projectionShader.setMat4("model", model);
	projectionShader.setVec3("aColor", glm::vec3(0.4f, 0.10f, 0.10f));
	glDrawElements(GL_TRIANGLES, 36, GL_UNSIGNED_INT, 0);
	...
	glActiveTexture(GL_TEXTURE);//textura3
	glBindTexture(GL_TEXTURE_2D, texture3);
	projectionShader.setInt("texture3", 2);
	...
	glActiveTexture(GL_TEXTURE);//textura2
	glBindTexture(GL_TEXTURE_2D, texture2);
	...
	model = glm::translate(model, glm::vec3(0.0f, .550f, 0.0f));
